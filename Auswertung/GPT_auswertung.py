...
DB_FORECAST = os.path.join(BASE_DIR, "gpt_predictions.db")    # forecast_header + forecast_lines
DB_ACTUAL   = os.path.join(BASE_DIR, "predictions.db")           # actual_prices (symbol,date,price)
    ...
    for _, h in headers.iterrows():
        ...
        for _, l in lines.iterrows():
            ...
            rows.append({
                "model": model,
                "symbol": sym,
                "created_at": created_day,  # Tagesdatum der Prognose
                "horizon": step,
