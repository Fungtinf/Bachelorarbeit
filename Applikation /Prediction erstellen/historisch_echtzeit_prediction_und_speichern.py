# C:\Bachelorarbeit\LSTM_Modell_prediction\historisch_echtzeit_prediction_und_speichern.py
import argparse, os, json, uuid, sqlite3, numpy as np, torch
...
# ================== CONFIG ==================
DEFAULT_CKPT      = r"C:\Bachelorarbeit\LSTM_historisch_trained\historisch_only_from_trainingsdaten_h7_symbol_emb\final_on_all\final_best_historisch_hparams.pt"
DEFAULT_OUT_DB    = r"C:\Bachelorarbeit\Datenbank\historisch_predictions.db"
...
MODEL_TYPE = "historisch"
MODEL_NAME = "Historisch_LSTM_live"

# ================== Symbol-Alias (für Embedding-Vokab) ==================
SYMBOL_ALIASES = {
    "Nestlé": "Nestle",
}
...
# ================== Modell (mit Symbol-Embedding) ==================
class LSTMSeqMultiHeadWithSym(nn.Module):
    """
    Training-kompatibel:
    - Input: Sequenz (T,F) + sym_id
...
# ================== Feature-Sequenz (historisch-only) ==================
def build_feature_sequence(symbol_name, ck, dates_iso, closes, market_db, market_tkr):
    """
    Features je Schritt (T=LOOKBACK-1):
      [ret, mkt_ret, mkt_ema, vol_std, vol_ewma, weekday_onehot(7)]
    Standardisierung mit ck['mean'] / ck['scale'].
    """
    T = LOOKBACK - 1
    F = ck["in_dim"]
...
def predict_one(ck, item, prices_db, market_db, market_tkr, source):
    ...
    # Kurse bis inkl. ENV holen
    as_of = env_as_of_date()
    dates_iso, closes = get_last_closes(prices_db, db_symbol, LOOKBACK, as_of)
    if not dates_iso:
        raise RuntimeError(f"Nicht genug Preise in actual_prices für {db_symbol} (LOOKBACK={LOOKBACK}).")
    ...
    # --- FIX: nur 1..6 nutzen, Tag 7 = Tag 6 (für Streamlit) ---
    if len(rel) >= 6:
    ...
    header = {
        ...
        "symbol": db_symbol,
        "close_t": base,
        "impact_1d": 0.0, "impact_3d": 0.0, "impact_7d": 0.0,
        "horizon": 7,
        "model_name": MODEL_NAME,
        "model_cfg_json": json.dumps(
            {"features": "historical-only: ret+mkt+ema+vol+weekday, sym_emb",
             "store_h7_as_h6": True},
            ensure_ascii=False
        )
    }
...
# ================== CLI ==================
def parse_args():
    ap = argparse.ArgumentParser("Historisch-only Echtzeit-Predictions -> historisch_predictions.db")
    ap.add_argument("--ckpt",      default=DEFAULT_CKPT)
    ap.add_argument("--db",        default=DEFAULT_OUT_DB)
    ap.add_argument("--json",      default=None, help="Items-JSON (Liste von Objekten)")
    ap.add_argument("--source",    default="historisch-live")
    ...
    if args.json and os.path.exists(args.json):
        ...
        for sym in symbols:
            if as_of:
                ...
                """, (sym,)).fetchone()
            if row:
                items.append({"symbol": sym, "close_t": float(row[0])})
