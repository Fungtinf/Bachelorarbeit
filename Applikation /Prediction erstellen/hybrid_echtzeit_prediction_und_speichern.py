# C:\Bachelorarbeit\LSTM_Modell_prediction\hybrid_echtzeit_prediction_und_speichern.py
...
DEFAULT_OUT_DB    = r"C:\Bachelorarbeit\Datenbank\hybrid_predictions.db"
...
MODEL_TYPE = "hybrid"
MODEL_NAME = "Hybrid_LSTM_live"

# ================== Modell (mit Symbol-Embedding) ==================
class LSTMSeqMultiHeadWithSym(nn.Module):
    """
    Muss exakt zur Trainings-Architektur passen:
    - Input: Sequenz (T,F) + sym_id
    - Output: Pfad (HORIZON) mit relativen Returns
    """
...
# ================== Feature-Bau (wie im HYBRID-Training) ==================
...
def build_feature_sequence(symbol_name, ck, dates_iso, closes, market_db, market_tkr,
    ...
    Feature-Layout (T,F) wie im HYBRID-Training:
      ...
      F = 5 + 3 + 7 = 15 (ohne Symbol-Onehot; Symbol kommt über Embedding)
    ...
    # Symbol-ID (mit <UNK>=0 Fallback)
    sym_id = torch.tensor([ck["stoi"].get(symbol_name, 0)], dtype=torch.long)
...
def predict_one(ck, item, prices_db, market_db, market_tkr, source):
    ...
    imp1 = float(item.get("impact_1d", 0.0))
    imp3 = float(item.get("impact_3d", 0.0))
    imp7 = float(item.get("impact_7d", 0.0))
    ...
    header = {
        ...
        "model_cfg_json": json.dumps(
            {
                "features": "hybrid: ret+mkt+ema+vol+imp(1,3,7)+weekday, sym_emb=<UNK>",
                "store_h7_as_h6": True
            },
            ensure_ascii=False
        )
...
# ================== CLI ==================
def parse_args():
    ap = argparse.ArgumentParser("Hybrid Echtzeit-Predictions (neues Embedding-Modell) -> hybrid_predictions.db")
    ap.add_argument("--ckpt",      default=DEFAULT_CKPT)
    ap.add_argument("--db",        default=DEFAULT_OUT_DB)
    ap.add_argument("--json",      default=None, help="Items-JSON (Liste von Objekten)")
    ap.add_argument("--source",    default="hybrid-live")
...
def main():
    ...
    print(f"Fertig. {saved}/{len(items)} Vorhersagen geschrieben nach {args.db}")
